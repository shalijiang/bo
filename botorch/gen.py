...
def gen_candidates_scipy(
    ...
    log_filepath = options.get("log_nit_nfev", None)
...
def gen_candidates_torch(
    ...
    scheduler = None
    if options.get("scheduler_on", False):
        def lr_lambda(iteration):
            return ((iteration + 1) / float(iteration)) ** (-options.get("gamma", 0.7))
        scheduler = MultiplicativeLR(bayes_optimizer, lr_lambda)
