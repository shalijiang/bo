...
r"""
Two-step expected improvement via one-shot optimization as introduced in
...
from .analytic import ExpectedImprovement
...
from ..sampling.samplers import MCSampler, SobolQMCNormalSampler, GaussHermiteSampler
from ..utils.transforms import match_batch_shape, t_batch_mode_transform

from .monte_carlo import qExpectedImprovement
from .objective import AcquisitionObjective, MCAcquisitionObjective


class TwoStepEI(qKnowledgeGradient):
    r"""two-step lookahead using one-shot optimization.
    ...
    def forward(self, X: Tensor) -> Tensor:
        ...
        if self.q1 == 1:  # two-step, use analytic EI
            ...
            value_function = qExpectedImprovementBatch(
                model=fantasy_model, sampler=self.inner_sampler, best_f=best_f.unsqueeze(-1)
            )
        ...
        if isinstance(self.sampler, GaussHermiteSampler):
            weighted_values = values * self.sampler.base_weights.view(-1, 1)
            future_utility = torch.sum(weighted_values, dim=0)
        else:
            future_utility = values.mean(dim=0)
        if self.scale_by_q1:
            future_utility = future_utility * (0.2 * self.q1 + 0.8)
